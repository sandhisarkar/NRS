...
namespace LItems
{
	...
	public class wfeImage: wItem, StateData
	{
        ...
        public DataSet GetAllExportedImagewithProj(string policy_number)
        {
            ...
            sqlStr = "select proj_key,batch_key,box_number,page_index_name,status,page_name,doc_type,policy_number,qc_size from image_master " +
        ...
        public ArrayList GetDeletedPage_List(eSTATES[] prmPolicyState, eSTATES[] prmImageState, wItem wi)
        {
            ...
            try
            {
                ...
                strQuery = "select distinct A.proj_key,A.batch_key,A.box_number,A.policy_number,A.page_name,A.doc_type from image_master A,metadata_entry B where A.proj_key=B.proj_code and A.batch_key=B.bundle_key and A.policy_number=B.patient_id and A.proj_key=" + queryPolicy.ctrlPolicy.ProjectKey + " and A.batch_key=" + queryPolicy.ctrlPolicy.BatchKey + " and A.box_number='" + queryPolicy.ctrlPolicy.BoxNumber + "' and A.policy_number='" + queryPolicy.ctrlPolicy.PolicyNumber + "' and A.status= " + (int)prmImageState[0];
        ...
        public ArrayList GetDeletedPageList3(eSTATES[] prmImageState, wItem wi)
        {
            ...
            try
            {
                ...
                //    }
                //}
                strQuery = strQuery + " or B.status ='7' or B.status ='31' or B.status ='30' or b.status = '37' or b.status = '40' or b.status = '77' or B.status = '8' or B.status=" + 5 + " or B.status=" + 6 + ")";
                wAdap = new OdbcDataAdapter(strQuery, sqlCon);
                wAdap.Fill(ds);
            }
            catch (Exception EX)
