...
namespace ImageHeaven
{
    ...
    public partial class aeImageQC : Form
    {
        ...
        public ArrayList GetItems(eITEMS item, eSTATES[] state)
        {
            ...
            {
                strQuery = "select proj_code,bundle_key,patient_id from metadata_entry where proj_code= '" + projKey + "' and bundle_key='" + bundleKey + "' and status ='2'";
        ...
        public ArrayList GetImagesItems(eITEMS item, eSTATES[] state, string case_file_no)
        {
            ...
            if (item == eITEMS.PAGE)
            {
                strQuery = "select distinct A.proj_key,A.batch_key,A.box_number,A.policy_number,A.page_name,A.page_index_name,A.doc_type from image_master A,metadata_entry B where A.proj_key=B.proj_code and A.batch_key = B.bundle_key  and A.policy_number = B.patient_id and A.photo <> 1 and A.proj_key=" + projKey + " and A.batch_key=" + bundleKey + " and  A.policy_number='" + case_file_no + "' and a.status <> 29 and b.status = 2 order by a.serial_no";
        ...
        public bool UpdateStatus(eSTATES state, Credentials prmCrd, bool pLock)
        {
            ...
            sqlStr = @"update metadata_entry" +
                " set status=" + 3 + ",modified_by='" + prmCrd.created_by + "',modified_dttm='" + prmCrd.created_dttm + "' where proj_code =" + ctrlPolicy.ProjectKey +
        ...
        public int GetFileCount(string projkey, string bundleKey)
        {
            ...
            sqlStr = "select patient_id from metadata_entry " +
                ...
                " and patient_id=" + bundleKey + " and status = '2'";
